...
# # Initialize Firebase
# cred = credentials.Certificate("./gradex-final-firebase-adminsdk-gcz1t-6fb1442ee2.json")
...
def simple_grade_answer(question, sample_answer, student_answer, max_marks):
    """
    Very simple grading function that uses Claude to evaluate and grade student answers
    without any complex NLP or metrics
    """
    # Generate feedback using Claude
    prompt = f"""You are an experienced, compassionate university teacher who genuinely cares about your students' learning and growth. Your goal is to provide constructive, encouraging feedback that helps students understand their mistakes and improve.
...
Based on the student's performance, assign a grade out of {max_marks} and an 'Answer Status' from the following categories: Correct, Partially Correct, Partially Incorrect, Incorrect.

Provide a detailed response in the following format:
Grade: [Assign a numerical grade out of {max_marks}]
Answer Status: [Select from: Correct, Partially Correct, Partially Incorrect, Incorrect]
Feedback:
[Provide detailed, encouraging feedback that explains why marks were given or deducted. Maximum 60 words.]
Correct Answer:
[Provide the full correct answer, including any important steps or explanations.]
Common Mistake:
...
Do strict checking dont give marks if the student has not written on the exact topic. Just tell them why they are wrong.
...
Remember to maintain a supportive and encouraging tone throughout your feedback, focusing on the student's learning journey rather than just the correctness of the answer."""
    ...
    print(f"Graded answer. Grade: {grade}, Answer Status: {answer_status}, Feedback: {feedback[:50]}...")
    ...
    for doc in docs:
        try:
            ...
            for i in range(len(data["questions"])):
                ...
                # Grade the student answer with the simple approach
                grading_result = simple_grade_answer(
                    question=question,
                    sample_answer=sample_answer,
                    student_answer=student_answer,
                    max_marks=max_marks
                ...
                # Store the result for this specific question
                graded_responses.append(grading_result)
